 ...
 */
class RobotImageSourceStream implements PullBufferStream
{
	private final static org.apache.log4j.Logger CLASS_LOGGER = org.apache.log4j.Logger.getLogger(RobotImageSourceStream.class);
	...
	public RobotImageSourceStream(AviCreator aviCreator, Rectangle rect, int frameRate)
	{
...
// Bug fix from Forums (next line)
		...
		format = new RGBFormat(new Dimension(rect.width,
				rect.height), rect.width*rect.height*3,
				Format.intArray,
				frameRate,
				32,
				...
				//new javax.media.format.RGBFormat(new Dimension(rect.width,
				//rect.height), Format.NOT_SPECIFIED, Format.intArray, frameRate, 32, bMask,
				//gMask, rMask);
	...
	public void read(Buffer buf) throws IOException
	{
		...
		if ((aviCreator!=null)&&(aviCreator.internalConstantFrameRate == false))
		{
			try
			{
...
// We are done. Set EndOfMedia.
		...
		try
		{
			frame = new Robot().createScreenCapture(rect);
		...
		if ((aviCreator==null)||(aviCreator.internalConstantFrameRate))
		{
		...
		else
		{
			...
			if ((aviCreator!=null)&&(aviCreator.timestampOfFirstFrame < 0))
